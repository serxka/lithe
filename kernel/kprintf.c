...
#define PUTC(c) do { writter(data, c); ++written; } while(0)
...
size_t print_hex(uint64_t val, bool upper, bool zeros, int (*writter)(void*, char), void *data) {
	size_t written = 0;
	
	uint32_t width = 1;
	uint64_t z = 0xF;
	while (val > z && z < UINT64_MAX) { // Test how width the number is so we don't pad with zero
		width += 1;
		z <<= 4;
		z |= 0xF;
	}
	...
	while (width-- > 0) {
		char c = (upper ? "0123456789ABCDEF" : "0123456789abcdef")[(val >> (width * 4)) & 0xF];
		PUTC(c);
	}
...
size_t wvaprintf(int (*writter)(void*, char), void *data, const char *fmt, va_list ap) {
	size_t written = 0;
	while (*fmt != '\0') {
		...
		switch (*fmt) {
			...
			case 'x': {
				...
				bool upper = !(*fmt == 'x');
				bool zeros = *fmt == 'p';
				written += print_hex(value, upper, zeros, writter, data);
				break; }
			case 'd': {
				...
				break; }
			case 'u': {
				unsigned int num = (unsigned int)va_arg(ap, unsigned int);
				written += print_dec(num, writter, data);
				break; }
			default:
				PUTC(*fmt);
